...
After the data are cleaned, shapefiles are created to have the data as spacially referenced. This takes the data for each GIS location and assigns it to a spacially referenced point. For this project, we want all data to be referenced to UTM Zone 18 North. This data was collected using World Geodetic System 1984. Note: CRS stands for Coordinate Reference System
...
The home range of each bear will be determined using an Minimum Convex Polygon. We will generate a 99% MCP (meaning generating an border around 99% of the data points). This ensures that if any extreme fixes were not removed during data cleaning, the home range calculation will not include the area where that point lies. We will also include a 200 meter buffer around the MCP. This is to include the area around the use locations that is still available for the bear to go. We chose 200 meters because that is how far a bear can travel in the allotted amount of time (talk about fixes and movement steps).
...
# usisng gsub removes the file extension for each row in the bear column
...
The data needs to be clipped to the Massachusetts state boundary. This is because the GIS layers used in this project are all in the extent of the MA border. If points were to extract spacial data outside the MA border, the results would be NA because there isn't spacial data outside the state.  
